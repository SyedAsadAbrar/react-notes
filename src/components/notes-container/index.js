...
import {getCurrentDateAndTime, getRandomNum} from "../../utils/common";
import {SentimentDissatisfied} from "@mui/icons-material";

const notesMock = [new NoteDTO("First Note", getCurrentDateAndTime(), true, loremIpsum({count: getRandomNum(1, 5)})), new NoteDTO("Second Note", getCurrentDateAndTime(), false, loremIpsum({count: getRandomNum(1, 5)})), new NoteDTO("Third Note", getCurrentDateAndTime(), false, loremIpsum({count: getRandomNum(1, 5)})), new NoteDTO("Fourth Note", getCurrentDateAndTime(), false, loremIpsum({count: getRandomNum(1, 5)}))];
...
const NotesContainer = () => {
    ...
    return (<Box sx={{mt: 2}}>
            <ClickAwayListener onClickAway={clickAwayHandler}>
                <Box>
                    {(isNotesInputFocused) && <TransitionGroup>
                        <Collapse key={"newNoteTitleCollapse"}>
                            <TextField fullWidth label="Title" id="noteTitle"
                                       variant={"filled"}
                                       value={newNoteTitle}
                    ...
                    <TextField fullWidth label="Take a note..." id="noteDetails" variant={"filled"}
                               ...
                               autoComplete={"off"}
                               InputProps={{
                                   endAdornment: isNotesInputFocused ? <InputAdornment position="end">
                                       <Button variant="text" color="primary"
                                               onClick={() => editNotes(null, actionTypes.ADD, {
                                                   title: newNoteTitle, note: newNote
                                               })}>Save</Button>
            ...
            <List>
                <TransitionGroup>
                    {notes.map((note, index) => <Collapse key={note.title}>
                        <Grid item xs={6}>
            ...
            {!notes.length && <Container sx={{textAlign: "center"}}>
